...
import { type ClientProfileViewPageOverviewTabProgramTrackRow_programTrack as Program } from './__generated__/ClientProfileViewPageOverviewTabProgramTrackRow_programTrack.graphql';

const useFragment = createFragment<Program>(graphql`
  fragment ClientProfileViewPageOverviewTabProgramTrackRow_programTrack on ProgramTrack
...
export default function ClientProfileViewPageOverviewTabProgramTrackRow(props: Props) {
  ...
  const onCardContentClick = React.useCallback(() => {
    history.push(
      `/dashboard/clients/view/${clientProfile.id}/tracks/view/${programTrack.id}/calendar`,
    );
